...
void buildMazeKruskal ()
{
    ...
    {
        int randomIndex = rand () % ( shuffleIndex + 1 );
...
void buildMazeDepthFirst ()
{
    ...
    int cellStack[MazeRows_Glob * MazeColumns_Glob];
    int stackSize = 0;
...
int main ()
{
    ...
    while ( MazeColumns_Glob <= 0 )
    {
        ...
        if ( fgets ( userInput, sizeof ( userInput ), stdin ) != NULL )
        {
            ...
            {
                for ( int characterIndex = 0; userInput[characterIndex] != '\0'; characterIndex = characterIndex + 1 )
                {
