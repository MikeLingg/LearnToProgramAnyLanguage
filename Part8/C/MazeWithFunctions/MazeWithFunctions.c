...
void printMaze()
{
    ...
    {
        bool* verticalWallsUp = malloc ( ( MazeColumns_Glob - 1 ) * sizeof ( bool ) );
        ...
        {
            bool* horizontalWallsUp = malloc ( MazeColumns_Glob * sizeof ( bool ) );
            ...
            free ( horizontalWallsUp );
        }
        
        free ( verticalWallsUp );
    }
...
void buildMazeKruskal()
{
    ...
    int** wallConnections = malloc ( InteriorWallCount_Glob * sizeof ( int* ) );
    for ( int i = 0; i < InteriorWallCount_Glob; i++ )
    {
        wallConnections[i] = malloc ( 2 * sizeof ( int ) );
    }
    
    int* wallRemoveList = malloc ( InteriorWallCount_Glob * sizeof ( int ) );
    int* cellToGroup = malloc ( MazeRows_Glob * MazeColumns_Glob * sizeof ( int ) );
    
    int** groupCells = malloc ( MazeRows_Glob * MazeColumns_Glob * sizeof ( int* ) );
    for ( int i = 0; i < MazeRows_Glob * MazeColumns_Glob; i++ )
    {
        groupCells[i] = malloc ( MazeRows_Glob * MazeColumns_Glob * sizeof ( int ) );
    }
    ...
    for ( int i = 0; i < InteriorWallCount_Glob; i++ )
    {
        free ( wallConnections[i] );
    }
    free ( wallConnections );
    free ( wallRemoveList );
    free ( cellToGroup );
...
int main()
{
    ...
    while ( MazeColumns_Glob <= 0 )
    {
        ...
        char userInput[100];
        char* result = fgets ( userInput, sizeof ( userInput ), stdin );
        if ( result != NULL )
        {
    ...
    if ( InteriorWallCount_Glob > 1000000 )
    {
        printf ( "OK your maze size request is insane and over any reasonable memory bounds, please try again!\n" );
        return 1;
    }
