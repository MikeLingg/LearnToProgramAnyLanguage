...
int main()
{
    ...
    // ERROR 7: Using string in switch (C doesn't support this)
    char text[] = "hello";
    switch ( text )
    {
        case "hello":
    ...
    // ERROR 13: Variable declaration in case without braces
    switch ( value )
    {
        case 1:
            int result = 10;
    ...
    // ERROR 18: Using 'True' or 'False' with capital letters (not defined in C without stdbool.h macros)
    if ( True )
    {
        printf( "Hello\n" );
    }
    
    // ERROR 19: Missing stdbool.h for bool type (if we remove the include)
    // This would require removing #include <stdbool.h> to test
