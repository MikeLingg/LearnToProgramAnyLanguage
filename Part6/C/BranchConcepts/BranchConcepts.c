...
int main()
{
    ...
    if ( ( isLoggedIn == true ) && ( strcmp( role, "admin" ) == 0 ) && ( strcmp( method, "POST" ) == 0 ) && ( isBanned == false ) && ( resourceIsAvailable == true ) )
    {
    ...
    {
        if ( strcasecmp( inputBuffer2, "false\n" ) == 0 || strcasecmp( inputBuffer2, "false" ) == 0 )
        {
            readBool = false;
            readValidBool = true;
        }
        else if ( strcasecmp( inputBuffer2, "true\n" ) == 0 || strcasecmp( inputBuffer2, "true" ) == 0 )
        {
            readBool = true;
            readValidBool = true;
        }
    ...
    size_t len = strlen( inputBuffer3 );
    if ( len > 0 && inputBuffer3[len-1] == '\n' )
    {
        inputBuffer3[len-1] = '\0';
    }
    ...
    {
        if ( strcasecmp( inputBuffer3, "false" ) == 0 )
        {
            readBool = false;
            readValidBool = true;
        }
        else if ( strcasecmp( inputBuffer3, "true" ) == 0 )
        {
            readBool = true;
            readValidBool = true;
        }
    ...
    bool inputMatchesFalse = ( strncmp( inputBuffer3, falseString, strlen( falseString ) ) == 0 );

    // Make sure both strings have the appropriate length
    strcpy( falseString, "false" );
    int subStringLength = strlen( falseString );

    if ( subStringLength > (int)strlen( inputBuffer3 ) )
    {
        subStringLength = strlen( inputBuffer3 );
    }

    inputMatchesFalse = ( strncmp( inputBuffer3, falseString, subStringLength ) == 0 );
    ...
    difference = fabsf( sum - thirdFloat );
